...
const char *
openord_get_stage_name (int stage_id)
{
  switch (stage_id)
    {
    case 0:
...
void
openord_init (OpenOrdContext *ctx, int node_count, int grid_size)
{
...
void
openord_cleanup (OpenOrdContext *ctx)
{
  if (ctx->density_grid)
...
static inline int
get_grid_idx (float x, float y, float z)
{
...
static void
update_density (OpenOrdContext *ctx, vec3 pos, float sign)
{
...
static float
get_density (OpenOrdContext *ctx, vec3 pos)
{
  int gx = (int)((pos[0] + HALF_VIEW) * VIEW_TO_GRID);
  int gy = (int)((pos[1] + HALF_VIEW) * VIEW_TO_GRID);
  int gz = (int)((pos[2] + HALF_VIEW) * VIEW_TO_GRID);
...
static float
compute_energy (OpenOrdContext *ctx, GraphData *g, int node_idx, vec3 pos)
{
  float energy = 0.0f;
  float attraction = ctx->stages[ctx->stage_id].attraction;
  float attraction_factor = attraction * attraction * attraction * attraction
                            * 2e-2f; // From original
...
static void
solve_analytic (OpenOrdContext *ctx, GraphData *g, int node_idx, vec3 out_pos)
{
  vec3 centroid = { 0, 0, 0 };
  ...
  if (total_weight > 0.0f)
    {
      ...
      vec3 current
          = { g->nodes[node_idx].position[0], g->nodes[node_idx].position[1],
...
bool
openord_step (OpenOrdContext *ctx, GraphData *graph)
{
  if (ctx->stage_id >= 5)
    return false;
  ...
  if (ctx->current_iter >= stage->iterations)
    {
      ...
      else if (ctx->stage_id == 3)
        { // Crunch
          ctx->cut_off_length = ctx->cut_end;
          ctx->min_edges = 1.0f;
        }
      else if (ctx->stage_id == 4)
        {                         // Simmer
