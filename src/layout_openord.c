...
void openord_init(OpenOrdContext* ctx, int node_count, int grid_size) {
    ...
    // Allocate density grid (can be huge, maybe use sparse if memory is issue, but 128^3 is 2M floats = 8MB, OK)
    // If user wants larger, we might need a hash map. For now, 128^3 is fast.
    ...
    if (cut_length_end <= 1.0f) cut_length_end = 1.0f;
...
void openord_cleanup(OpenOrdContext* ctx) {
    if (ctx->density_grid) free(ctx->density_grid);
...
static inline int get_grid_idx(float x, float y, float z) {
    ...
    if (gx < 0) gx = 0; if (gx >= GRID_DIM) gx = GRID_DIM - 1;
    if (gy < 0) gy = 0; if (gy >= GRID_DIM) gy = GRID_DIM - 1;
    if (gz < 0) gz = 0; if (gz >= GRID_DIM) gz = GRID_DIM - 1;
...
static float get_density(OpenOrdContext* ctx, vec3 pos) {
    ...
    if (gx < 0 || gx >= GRID_DIM || gy < 0 || gy >= GRID_DIM || gz < 0 || gz >= GRID_DIM) return 10000.0f;
    
    int idx = gz * GRID_DIM * GRID_DIM + gy * GRID_DIM + gx;
    return ctx->density_grid[idx]; // Simplified, originally squared but linear is often fine for 3D repulsion
}
...
static float compute_energy(OpenOrdContext* ctx, GraphData* g, int node_idx, vec3 pos) {
    ...
    for (int i = 0; i < igraph_vector_int_size(&neighbors); i++) {
        ...
        if (ctx->stage_id < 2) dist_sq *= dist_sq; // Higher power for liquid/expansion
        if (ctx->stage_id == 0) dist_sq *= dist_sq; // Liquid
...
bool openord_step(OpenOrdContext* ctx, GraphData* graph) {
    if (ctx->stage_id >= 5) return false;
    ...
    for (int i = 0; i < graph->node_count; i++) {
        ...
        // We can do approximate density updates or use atomics in density grid (which we added).
        
        vec3 old_pos; glm_vec3_copy(graph->nodes[i].position, old_pos);
    ...
    if (ctx->stage_id == 1) { // Expansion
        if (stage->attraction > 1) stage->attraction -= 0.05f;
        if (ctx->min_edges > 12) ctx->min_edges -= 0.05f;
        ctx->cut_off_length -= ctx->cut_rate;
        if (stage->damping_mult > 0.1f) stage->damping_mult -= 0.005f;
    } else if (ctx->stage_id == 2) { // Cooldown
        if (stage->temperature > 50) stage->temperature -= 10.0f;
        if (ctx->cut_off_length > ctx->cut_end) ctx->cut_off_length -= ctx->cut_rate * 2.0f;
        if (ctx->min_edges > 1) ctx->min_edges -= 0.2f;
    } else if (ctx->stage_id == 4) { // Simmer
        if (stage->temperature > 50) stage->temperature -= 2.0f;
    }
