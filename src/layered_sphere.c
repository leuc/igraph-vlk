...
int
compare_communities_kcore (const void *a, const void *b)
{
...
int
compare_nodes_placement (const void *a, const void *b)
{
...
int
get_vector_int_max (const igraph_vector_int_t *v)
{
  int max_val = 0;
...
int
find_closest_slot_by_hilbert (SphereGrid *grid, int target_h)
{
  ...
  int best_idx = low;
  if (low > 0
      && abs (grid->slots[low - 1].hilbert_dist - target_h)
...
// ---------------------------------------------------------
double
calculate_move_delta_intra (const igraph_t *ig, const igraph_matrix_t *layout,
...
// ---------------------------------------------------------
double
calculate_move_delta_inter (const igraph_t *ig, const igraph_matrix_t *layout,
                            LayeredSphereContext *ctx, int u,
                            int target_sphere_s, int target_slot_k)
{
  int v = ctx->grids[target_sphere_s].slot_occupant[target_slot_k];
...
void
layered_sphere_init (LayeredSphereContext *ctx, int node_count)
{
...
void
layered_sphere_cleanup (LayeredSphereContext *ctx)
{
  if (ctx->initialized)
    {
...
bool
layered_sphere_step (LayeredSphereContext *ctx, GraphData *graph)
{
  ...
  if (ctx->phase == PHASE_INIT)
    {
      ...
      // Collapse directed edges (A->B and B->A become a single A-B edge). NULL
      // drops attributes.
      ...
      for (int s = 0; s < ctx->num_spheres; s++)
        {
          ...
          if (s == 0)
            ...
            {
              // THE SHELLS: Add a logarithmic structural gap from the previous
              // sphere
...
const char *
layered_sphere_get_stage_name (LayeredSphereContext *ctx)
{
