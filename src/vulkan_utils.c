  ...
  VkMemoryAllocateInfo allocInfo
      = { .sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
          ...
          .memoryTypeIndex = findMemoryType (
              physicalDevice, memReqs.memoryTypeBits, properties) };
  vkAllocateMemory (device, &allocInfo, NULL, bufferMemory);
  vkBindBufferMemory (device, *buffer, *bufferMemory, 0);
}

void
updateBuffer (VkDevice device, VkDeviceMemory memory, VkDeviceSize size,
              const void *data)
{
  void *mapped;
...
void
createImage (VkDevice device, VkPhysicalDevice physicalDevice, uint32_t width,
...
void
transitionImageLayout (VkDevice device, VkCommandPool commandPool,
...
VkResult
create_shader_module (VkDevice device, const char *path,
