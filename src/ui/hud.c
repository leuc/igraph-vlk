 ...
 */
static const char *centrality_names[] = {"PageRank", "Hubs", "Authorities", "Betweenness", "Degree", "Closeness", "Harmonic", "Eigenvector", "Strength", "Constraint"};
...
void ui_hud_update(AppState *state, float fps)
{
	...
	if (state->app_ctx.current_state == STATE_MENU_OPEN) {
		snprintf(menu_state, 512, " [MENU:%d]", state->renderer.menuNodeCount);
	...
	snprintf(buf, sizeof(buf),
			 ...
			 layout_names[state->current_layout], stage_info, comm_arrangement_names[state->current_comm_arrangement], cluster_names[state->current_cluster], centrality_names[state->current_centrality], state->renderer.showLabels ? "ON" : "OFF", state->current_graph.props.node_count, state->current_graph.props.edge_count, state->current_graph.props.coreness_filter, fps, menu_state);
