...
int
renderer_init (Renderer *r, GLFWwindow *window, GraphData *graph)
{
  r->window = window;
  r->currentFrame = 0;
  ...
  VkBufferImageCopy bReg
      = { .bufferOffset = 0,
          ...
          .imageExtent
          = { (uint32_t)globalAtlas.width, (uint32_t)globalAtlas.height, 1 } };
  ...
  for (int i = 0; i < PLATONIC_COUNT; i++)
    {
      ...
      updateBuffer (r->device, r->vertexBufferMemories[i],
                    sizeof (Vertex) * vc, v);
  ...
  for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
    {
      VkDescriptorBufferInfo bi
          = { r->uniformBuffers[i], 0, sizeof (UniformBufferObject) };
      ...
      VkWriteDescriptorSet dw[] = {
        ...
        { VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, NULL, r->descriptorSets[i],
          1, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &ii, NULL, NULL }
      };
...
void
renderer_update_view (Renderer *r, vec3 pos, vec3 front, vec3 up)
{
  vec3 c;
...
void
renderer_draw_frame (Renderer *r)
{
