...
int pipeline_ui_create(Renderer *r)
{
	...
	VkPipelineShaderStageCreateInfo mstages[] = {{VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, NULL, 0, VK_SHADER_STAGE_VERTEX_BIT, mVMod, "main", NULL}, {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, NULL, 0, VK_SHADER_STAGE_FRAGMENT_BIT, mFMod, "main", NULL}};

	// Vertex input for menu: QuadVertex (binding 0, vertex rate) + MenuInstanceData (binding 1, instance rate)
	VkVertexInputBindingDescription mb[] = {{0, sizeof(QuadVertex), VK_VERTEX_INPUT_RATE_VERTEX}, {1, sizeof(MenuInstanceData), VK_VERTEX_INPUT_RATE_INSTANCE}};

	VkVertexInputAttributeDescription ma[] = {{0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(QuadVertex, pos)}, {1, 0, VK_FORMAT_R32G32_SFLOAT, offsetof(QuadVertex, uv)}, {2, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(MenuInstanceData, position)}, {3, 1, VK_FORMAT_R32_SFLOAT, offsetof(MenuInstanceData, scale)}, {4, 1, VK_FORMAT_R32_SINT, offsetof(MenuInstanceData, icon_index)}};

	VkPipelineVertexInputStateCreateInfo mvi = {.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO, .vertexBindingDescriptionCount = 2, .pVertexBindingDescriptions = mb, .vertexAttributeDescriptionCount = 5, .pVertexAttributeDescriptions = ma};

	VkPipelineColorBlendAttachmentState mcb = lcb; // Same blending
	VkPipelineColorBlendStateCreateInfo mcs = lcs;

	VkGraphicsPipelineCreateInfo mpInfo = {.sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO, .stageCount = 2, .pStages = mstages, .pVertexInputState = &mvi, .pInputAssemblyState = &lias, .pViewportState = &vpS, .pRasterizationState = &ras, .pMultisampleState = &mul, .pColorBlendState = &mcs, .layout = r->pipelineLayout, .renderPass = r->renderPass};
