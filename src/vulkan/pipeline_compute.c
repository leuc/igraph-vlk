...
int pipeline_compute_create(Renderer *r)
{
	...
	VkResult result = vkCreateDescriptorSetLayout(device, &cLayInfo, NULL, &r->computeDescriptorSetLayout);
	...
	result = vkCreatePipelineLayout(device, &cPlyLayInfo, NULL, &r->computePipelineLayout);
	if (result != VK_SUCCESS) {
