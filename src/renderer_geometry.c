...
typedef struct { int sourceId; int targetId; int elevationLevel; int pathLength; vec4 path[6]; } CompEdge;
...
void renderer_update_graph(Renderer* r, GraphData* graph) {
    ...
    int segments = (graph->active_layout == LAYOUT_LAYERED_SPHERE) ? 5 : 1; r->edgeVertexCount = r->edgeCount * segments * 2;
    ...
    if (graph->active_layout == LAYOUT_LAYERED_SPHERE && graph->layered_sphere && graph->layered_sphere->initialized) {
        ...
        for(uint32_t i=0; i<graph->node_count; i++) {
            ...
            cNodes[i].pad1 = 0; memcpy(cNodes[i].color, graph->nodes[i].color, sizeof(vec3));
            cNodes[i].size = graph->nodes[i].size; cNodes[i].degree = graph->nodes[i].degree;
        ...
        vkDestroyBuffer(r->device, nBuf, NULL); vkFreeMemory(r->device, nMem, NULL);
        vkDestroyBuffer(r->device, eBuf, NULL); vkFreeMemory(r->device, eMem, NULL);
        // --- END GPU DISPATCH ---
