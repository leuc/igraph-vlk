...
typedef struct
{
  ...
  int pathLength;
  vec4 path[16];
  float animation_progress; // For routed edges
  int animation_direction;  // For routed edges
  int is_animating;         // For routed edges
} CompEdge;
typedef struct {
	float position[3];
	float pad;
...
void renderer_update_graph(Renderer *r, GraphData *graph) {
	...
	if (r->currentRoutingMode != ROUTING_MODE_STRAIGHT) {
		...
		for (uint32_t i = 0; i < graph->edge_count; i++) {
			...
			// For routed edges, calculate normalized_pos based on segment length
			// This is an approximation: assuming uniform segment lengths
			float total_length = 0.0f;
			if (pLen > 1) {
				for (int p = 0; p < pLen -1; ++p) {
					vec3 p1, p2;
					p1[0] = cEdges[i].path[p][0]; p1[1] = cEdges[i].path[p][1]; p1[2] = cEdges[i].path[p][2];
					p2[0] = cEdges[i].path[p+1][0]; p2[1] = cEdges[i].path[p+1][1]; p2[2] = cEdges[i].path[p+1][2];
