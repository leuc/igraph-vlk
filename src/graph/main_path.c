...
void apply_main_path_weighting(ExecutionContext *ctx, void *result_data)
{
	...
	if (!renderer_init_criticality_buffers(&state->renderer, graph, &prep->levels, prep->num_levels, prep->weight_mode) || !renderer_start_main_path_weighting(&state->renderer, graph, &prep->levels, ctx->follow_animation_duration))
