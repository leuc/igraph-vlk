...
void graph_calculate_centrality(GraphData *data, CentralityType type)
{
	...
	case CENTRALITY_PAGERANK:
		igraph_pagerank(&data->g, NULL, &result, NULL, 0.85, IGRAPH_DIRECTED, igraph_vss_all(), IGRAPH_PAGERANK_ALGO_PRPACK, NULL);
		break;
	case CENTRALITY_HUB: {
		// For undirected graphs, hub scores are same as eigenvector centrality
		if (!igraph_is_directed(&data->g)) {
			igraph_eigenvector_centrality(&data->g, &result, NULL, IGRAPH_ALL, NULL, NULL);
		} else {
			igraph_vector_t a;
			igraph_vector_init(&a, 0);
			igraph_hub_and_authority_scores(&data->g, &result, &a, NULL, NULL, NULL);
	...
	case CENTRALITY_AUTHORITY: {
		...
		if (!igraph_is_directed(&data->g)) {
			...
			igraph_hub_and_authority_scores(&data->g, &h, &result, NULL, NULL, NULL);
			igraph_vector_destroy(&h);
		}
		break;
	}
	case CENTRALITY_BETWEENNESS:
		igraph_betweenness(&data->g, NULL, &result, igraph_vss_all(), IGRAPH_DIRECTED, 1);
		break;
	...
	case CENTRALITY_CLOSENESS: {
		...
		igraph_closeness_cutoff(&data->g, &result, NULL, &all_reach, igraph_vss_all(), IGRAPH_ALL, NULL, 1, 0);
		break;
	}
	case CENTRALITY_HARMONIC:
		igraph_harmonic_centrality(&data->g, &result, igraph_vss_all(), IGRAPH_ALL, NULL, 1);
		break;
	case CENTRALITY_EIGENVECTOR:
		igraph_eigenvector_centrality(&data->g, &result, NULL, IGRAPH_ALL, NULL, NULL);
		break;
	case CENTRALITY_STRENGTH:
		igraph_strength(&data->g, &result, igraph_vss_all(), IGRAPH_ALL, IGRAPH_LOOPS, NULL);
		break;
